...
骨架里每个待写段落都带【待写】标记，validate_report.py 把该标记判为硬失败，
因此骨架不可能被当成报告交付；要出门必须先把标记写没。
"""
...
def build_skeleton(ledger_dir: str, title: str = '') -> str:
    ...
    for topic in order:
        ...
        for c in [x for x in claims if x['topic'] == topic]:
            ...
            if c['status'] == 'verified':
                ...
                lines.append(f"- ⚠️ 待核：{c['text']} {cite}"
                             f"（状态 {c['status']}，未达 verified 判据）")
                lines.append(f'  {PLACEHOLDER} 补交叉验证/一手来源，或降为"仅作线索"')
        lines.append('')

    lines += ['## 调研方法',
              f'{PLACEHOLDER} 写检索窗口、数据源分层、子 Agent 分工与 verified 判据（档 A/档 B）',
              f'账本事实：claims {len(claims)} 条（verified {verified}），'
    ...
    lines += ['', '---',
              ...
              '标了"待写"的段落必须由 Lead 写掉——标记不删净就过不了校验门，'
              '骨架也就永远不会被当成报告交付。']
    return '\n'.join(lines) + '\n'
