...
**已修 13 条**（1、2、4、5、6、7、9、10、11、12、13、15、16），**部分处理 2 条**（3 只补了文档口径、
14 已离线复测定位（见 ⑭），剩下的是引擎侧取证），
...
**测试**：新增 `tests/test_v615_dogfood.py` 34 项（重填已有 URL 不升级、arXiv 四种表示算同一制品、
...
缺失详情要带实测后果与修复命令、requirements.txt 不许再写"不影响基本功能"）。"不同分支是不同制品"由既有测试
`test_v66_fixes.py::test_different_branch_is_a_different_artifact` 守着；另有三条旧测试
原先依赖"同一 URL 重填即可升级"这条路径，改为使用真正不同的制品表示，规则本身未放宽。
`scripts` 349 passed，根 `tests` 49 passed（`tests/test_gate_hook.py` 这轮 +4 项：没得判时一声不出、
判过就要留痕、CLI 形状的会话会被判到而不是跳过、盖戳后改下层账本仍拦得住）。

**本轮未做，留清单**：
第 3 条的"探针按主题探活"仍未写代码——本次翻案恰恰证明它必要：`--probe` 在装前（全 406）和装后
（真出数据）给出的判定一模一样，闸门看不见传输层被拦；
