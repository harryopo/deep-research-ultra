...
## v6.33.1（2026-09-26）— 签名表漂移测试改成横扫所有 CLI，又抓出 panel.py 两条

上一条只盯 `ledger.py`。按"抓到一处就横扫同类"重扫全部带子命令的脚本，
`panel.py` 的 **`supplement-questions` 与 `review-draft` 同样只在脚本自己的用法文本里，
§14.3 那张"照抄即可"的签名块里没有**——等于专家团的两项能力对照着签名表干活的人不存在。

- 防漂测试改为遍历 `scripts/*.py`，凡有 `cmd == '<子命令>'` 分派的都纳入断言，
  并先要求"至少扫到一个带子命令的脚本"，防止扫描口径坏掉时测试空转通过。
- §14.3 补上这两条签名（`--input <file|text>` / `--role <role>` 照抄 panel.py 自己的用法文本）。
- 全量 `715 passed`（用例数不变，覆盖面从 10 条子命令扩到 14 条）。

---

## v6.33.0（2026-09-26）— §14.3 签名表补齐两条判同命令，并加了一道防漂测试

`SKILL.md` §14.3 的注释原话是「证据账本（写命令签名照抄，不必再跑 --help）」，冷启动那节也写着
"命令照抄即可（十四节的签名表已列全）"。实际对不上：v6.21 加的 `link-identity` 与
v6.32 加的 `content-identity` **只在正文散文里出现过，签名块里没有**——
照签名块抄命令的 Lead 不知道有这两条通道，"跨标识符 / 跨主机判同"这条升级路等于不存在，
归属型 claim 只能停在 pending（本轮实跑就是这么卡的）。

- §14.3 补上两条签名（含 `--anchor` / `--target` 与各自的拒绝条件一行注释）。
- 新增 `tests/test_v6330_signature_table.py`（2 条）：**从 `ledger.py` 的 CLI 分派里枚举子命令，
  逐个要求在 §14.3 那个 bash 块里出现**；"在 SKILL.md 别处提过"不算数。
  第二条是对照面，防止断言因为"块没解析到"而空转通过。
- 全量 `715 passed`（+2）。
- 顺手按实测定了一条冒烟测试的超时：GBK 控制台那三条命令都离线（`--list` 走
  `only_available=False`，不探测引擎可用性），单跑 5-6 秒，但一次全量运行里被同轮
  真实网络测试拖过 120 秒上限，报成假红（单跑立刻绿）。这条测的是编码不是网速，
  上限提到 600 秒，让它只在真挂住时说话。

**升级动作**：无配置变更，纯文档与守护测试。以后给 `ledger.py` 加子命令，签名表漏了会直接红。
